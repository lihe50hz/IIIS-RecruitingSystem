...
- LTZ同学自学 Scala 后将同文自动生成的项目精简整理为目前的架构，此后的前端和后端 Core 均在此基础上修改开发，后续开发过程完全手工进行。

- 第一周周末和第二周前半周，LTZ、LJY、LH负责登录、注册以及学生、教师账号的后端，MXC负责相应部分的前端界面。

- 第二周后半周，LJY、LH开发了管理员以及部分考试、消息功能的后端，并搭配基本的前端界面；LTZ负责连接前后端通信并且实现了邮箱验证码、找回密码等功能；MXC负责代码融合规范、前端美化和前端小功能。

- 第二周周末和第三周前半周，LJY、LH主要完善管理员、考试和消息的功能；LTZ实现了考试 markdown 支持，引入了流式布局，并大量改进后端实现；MXC负责代码融合精简、并在LTZ协助下完成第一次页面重构。

- 第三周后半周和周末，LJY、LH实现了水印功能和完善的用户、考试管理；LTZ完成了用户身份鉴权和前端调整；MXC在LTZ协助下完成第二次页面重构，设计了一些视觉效果，并主要负责 Demo 制作和报告写作。

## 前端设计

- 前端的第一版本（主要是登录部分）模仿 <https://admission.join-tsinghua.edu.cn/frontend/site/login> 的风格，希望达到戏谑的效果，并降低同学的学习成本。同时MXC使用了大量该网站的素材，忠实地按照清华大学的紫色风格设计。第一版本的前端主要基于LTZ在最开始使用的 <tsx react mui\> 书写，使用 <Box/\> 和固定像素大小的设计。这一设计在LJY、LH实现前端基本功能时得到沿用。

- 第二版本在LTZ的提议下全面拥抱流式布局<View/\>。这一阶段主要目标是将所有功能连接起来，并能正确地显示在网页上。同时，用户界面采用了一种偏浅色的界面，搭配清华紫的按钮，显得不甚和谐。
